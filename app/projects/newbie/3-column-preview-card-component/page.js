...
const items = [
    {
        title: "Sedans",
        icon: SedansIcon,
        className: "bg-(--bright-orange)",
        buttonClassName: "text-(--bright-orange)",
    ...
    },
    {
        ...
        buttonClassName: "text-(--dark-cyan)",
        description: "Take an SUV for its spacious interior, power, and versatility. Perfect for your next family vacation and off-road adventures."
    },
    {
        title: "Luxury",
        icon: LuxuryIcon,
        className: "bg-(--very-dark-cyan)",
        buttonClassName: "text-(--very-dark-cyan)",
...
function StylesSetup({ children }) {
    return <div className={`w-full min-h-screen px-6 py-[88px] grid place-items-center bg-(--very-light-gray) ${lexendDeca.className}`}
        style={{
...
function Card({ className, buttonClassName, title, icon, description }) {
    ...
    const buttonStyles = cn(
        "px-8 cursor-pointer py-3 bg-(--very-light-gray)",
        "rounded-full text-[15px] z-20",
        "border-2 border-(--very-light-gray)",
        "transition-colors hover:bg-transparent hover:text-(--very-light-gray)",
        buttonClassName)
    ...
    return <div className={containerStyles}>
        ...
        <h2 className={`text-(--very-light-gray) text-[40px] font-bold z-20 ${bigShouldersDisplay.className}`}>
            {title}
        </h2>
        <p className="text-(--transparent-white) text-[15px] leading-[25px] max-w-[212px] z-20">{description}</p>
