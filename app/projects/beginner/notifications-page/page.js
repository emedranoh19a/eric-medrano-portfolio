...
export default function Page() {
    ...
    return <StylesSetup>
        <div className="w-full max-w-sm md:max-w-xl h-fit bg-white lg:mb-48 shadow-xs rounded-lg p-4">
            <div className="w-full flex justify-between mb-2.5">
                <h2 className="text-lg font-bold text-(--grayish-blue-900)">
                    Notifications
                    <span className="bg-(--blue) p-1 px-2 ml-2 rounded-md text-white text-xs">{unreadNumber}</span>
                </h2>
                <button onClick={handleUnread} className="text-sm text-(--grayish-blue-300) font-bold hover:text-(--blue) transition-colors">Mark all as read</button>
            </div>
