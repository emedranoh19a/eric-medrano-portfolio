...
function Header({ balance = 921.48 }) {
    return <div className="bg-(--soft-red) p-5 rounded-xl flex justify-between items-center">
        <div className=" flex flex-col text-(--very-pale-orange)">
...
function Chart({ data }) {
    ...
    return <div className="bg-(--very-pale-orange) text-(--medium-brown) p-5 rounded-xl flex flex-col gap-4">
        <div >
        ...
        <div className="-mt-4 w-full flex justify-between items-end">
            <div>
                <span className="text-3xl text-(--dark-brown) font-bold">
                    $478.33
                </span>
            </div>
            <div>
                <span className="block text-(--dark-brown) font-bold"> +2.4% </span>
...
function Bar({ dayLabel, amount, maxAmount, index }) {
    ...
    const barStyles = cn(
        ...
        hasMaxData ? "bg-(--cyan)" : "bg-(--soft-red)")
        ...
        <div className="h-full relative">
            ...
            >
                <div className="relative w-full h-full">
                    <div className=" opacity-0 transition group-hover:scale-[1.1] group-hover:opacity-100 absolute text-sm bg-(--dark-brown) text-(--very-pale-orange) -translate-y-full -top-2 left-1/2 -translate-x-1/2 p-2 rounded-lg">
                        ${amount}
                    </div>
                </div>
            </motion.div>
