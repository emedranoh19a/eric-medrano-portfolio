...
export default function Input({
  ...
  const inputStyles = clsx(
    "appearance-none resize-none outline-none bg-transparent transition-colors",
    Tag === "textarea" && "h-40",
    "pb-4 border-b-2",
    hasError
      ? "border-b-[var(--error)]"
      : "border-b-[var(--green)] focus:border-b-[var(--green)] placeholder-shown:border-b-white/50",
...
function ErrorMessage({ message }) {
  return (
    <span className="text-right text-[var(--error)] absolute right-0 top-full translate-y-[5px] font-medium text-[12px] leading-[16px] tracking-[-0.17px]">
      {message}
    </span>
  );
}
